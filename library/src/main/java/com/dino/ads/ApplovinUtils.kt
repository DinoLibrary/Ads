...
object ApplovinUtils : LifecycleObserver {
    ...
    fun initApplovin(
        ...
        isClickAds = false
        this.enableAds = isEnableAds
        val executor = Executors.newSingleThreadExecutor()
        executor.execute {
            val initConfigBuilder =
                AppLovinSdkInitializationConfiguration.builder(sdkKey, application)
        ...
        if (interstitialAd.isReady) {
            activity.lifecycleScope.launch {
                if (dialogShowTime > 0) {
                    ...
                    activity.lifecycle.addObserver(DialogLifecycleObserver(dialog))
    ...
    fun loadInterstitial(
        ...
        interHolder.inter = MaxInterstitialAd(interHolder.adsId, context as Activity?)
    ...
    fun showBanner(
        ...
        banner = MaxAdView(adId, activity)
    ...
    @Deprecated("")
    fun loadAndGetNative(activity: Activity, adId: String, adCallback: NativeCallback) {
        if (!enableAds || !isNetworkConnected(activity)) {
    ...
    fun dialogLoading(activity: Activity) {
